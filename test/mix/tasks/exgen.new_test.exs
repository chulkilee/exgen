  ...
  describe "exgen.new without config" do
    ...
    test "generates app in current dir", %{template: template} do
      in_tmp(fn ->
        ...
        assert_rendered_template(
          ...
          "some_app/lib/some_app.ex",
          context
        )

        assert_rendered_template(
          "#{template}/lib/<%= app_name %>/router.ex",
          "some_app/lib/some_app/router.ex",
          context
        )
      end)
    end
    ...
    test "generates app in relative dir", %{template: template} do
      in_tmp(fn ->
        ...
        assert_rendered_template(
          "#{template}/lib/<%= app_name %>.ex",
          "test/some_app/lib/some_app.ex",
          context
        )

        assert_rendered_template(
          "#{template}/lib/<%= app_name %>/router.ex",
          "test/some_app/lib/some_app/router.ex",
          context
        )
      end)
    end
  ...
  describe "exgen.new with dynamic switches" do
    test "parses dynamic switches" do
      args = [
        "some_app",
        "-t",
        "https://github.com/rwdaigle/exgen-plug-simple.git",
        "--app-name",
        "some_app",
        "--module",
        "SomeApp"
